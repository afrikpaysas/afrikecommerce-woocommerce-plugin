...
if ( isset ($_GET["master_card"])) {
class WC_Gateway_MASTER_CARD extends WC_Payment_Gateway {}
?>
<form id="dataFormMASTER_CARD" action="<?php echo $_GET["urlafrikpay"]; ?>" method="post" target="_top">
<input type="hidden" name="provider" value="master_card"/>
...
<script type="text/javascript">
    document.getElementById('dataFormMASTER_CARD').submit(); // SUBMIT FORM
</script>
<?php

} else {
/**
 * WC_Gateway_MASTER_CARD class.
 *
...
class WC_Gateway_MASTER_CARD extends WC_Payment_Gateway {
	...
	public function __construct() {
        ...
        add_action('woocommerce_api_wc_gateway_master_card', [$this, 'pay']);
	...
	public static function log( $message, $level = 'info' ) {
		if ( self::$log_enabled ) {
			...
			self::$log->log( $level, $message, array( 'source' => 'master_card' ) );
		}
	}
	...
	protected function get_icon_image( $country ) {
		switch ( $country ) {
			default :
				$icon = WC_HTTPS::force_https_url( 'https://cloud.afrikpay.com/images/partners/master.png' );
			break;
		}
		return apply_filters( 'woocommerce_master_card_icon', $icon );
	}
	...
	public function is_valid_for_use() {
		return in_array( get_woocommerce_currency(), apply_filters( 'woocommerce_master_card_supported_currencies', array( 'XAF','USD','EUR' ) ) );
	}
	...
	public function init_form_fields() {
		$this->form_fields = include( 'settings-master_card.php' );
	}
	...
	public function process_payment( $order_id ) {
		include_once( 'class-wc-gateway-master_card-request.php' );

		$order          = wc_get_order( $order_id );
		$master_card_request = new WC_Gateway_MASTER_CARD_Request( $this );

		return array(
			'result'   => 'success',
			'redirect' => $master_card_request->get_request_url( $order ),
		);
	}
	...
	public function capture_payment( $order_id ) {
		$order = wc_get_order( $order_id );
		if ( 'master_card' === $order->get_payment_method() && 'pending' === get_post_meta( $order->get_id(), '_master_card_status', true ) && $order->get_transaction_id() ) {
...
$error=$_GET["error"];
			...
			if ( ! empty( $status ) ) {
				switch ( $status ) {
					case 'OK' :
						$order->add_order_note( sprintf( __( "Le paiement s'est bien pass\E9: %1$s", 'woocommerce' ), $status ) );
						update_post_meta( $order->get_id(), '_master_card_status', $status );
						update_post_meta( $order->get_id(), '_transaction_id', $status );
					break;
					default :
						$order->add_order_note( sprintf( __( "Le paiement ne s'est pas bien pass\E9: %1$s", 'woocommerce' ), $status ) );
	...
	public function admin_scripts() {
		...
		wp_enqueue_script( 'woocommerce_master_card_admin', '../wp-content/plugins/afrikecommerce-payments-for-woocommerce/assets/js/afrikpay-admin' . $suffix . '.js', array(), WC_VERSION, true );
	}
}
}
