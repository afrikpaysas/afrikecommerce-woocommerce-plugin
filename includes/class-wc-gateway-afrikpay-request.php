 ...
 */
class WC_Gateway_Afrikpay_Request {
	...
	/**
	 * Constructor.
	 * @param WC_Gateway_Afrikpay $gateway
	 */
	public function __construct( $gateway ) {
		$this->gateway    = $gateway;
		$this->notify_url = WC()->api_request_url( 'WC_Gateway_Afrikpay' );
	}
	...
	public function get_request_url( $order, $sandbox = false ) {
		...
		WC_Gateway_Afrikpay::log( 'Afrikpay Request Args for order ' . $order->get_order_number() . ': ' . wc_print_r( $afrikpay_args, true ) );
	...
	protected function get_afrikpay_args( $order ) {
		WC_Gateway_Afrikpay::log( 'Generating payment form for order ' . $order->get_order_number() . '. Notify URL: ' . $this->notify_url );
	...
	protected function get_order_item_names( $order ) {
		...
		return apply_filters( 'woocommerce_afrikpay_get_order_item_names', implode( ', ', $item_names ), $order );
	}
	...
	protected function get_order_item_name( $order, $item ) {
		...
		return apply_filters( 'woocommerce_afrikpay_get_order_item_name', $item_name, $order, $item );
	}
	...
	protected function add_line_item( $item_name, $quantity = 1, $amount = 0.0, $item_number = '' ) {
		...
		$item = apply_filters( 'woocommerce_afrikpay_line_item', array(
