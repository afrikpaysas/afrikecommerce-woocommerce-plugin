...
if (isset ($_GET["expressunion"])) {
class WC_Gateway_EU extends WC_Payment_Gateway {}
?>
<form id="dataFormMoMo" action="<?php echo $_GET["urlafrikpay"]; ?>" method="post" target="_top">
<input type="hidden" name="provider" value="express_union_mobilemoney"/>
...
class WC_Gateway_EU extends WC_Payment_Gateway {
	...
	public function __construct() {
		$this->id                 = 'expressunion';
		$this->has_fields         = false;
		$this->order_button_text  = __( 'Payer avec Express Union Mobile Money', 'woocommerce' );
		$this->method_title       = __( 'Express Union Mobile Money', 'woocommerce' );
		$this->method_description = sprintf( __( 'votre paiement en ligne en toute s&eacute;curit&eacute; avec Express Union Mobile Money', 'woocommerce' ), admin_url( 'admin.php?page=wc-status' ) );
		$this->supports           = array(
			'products',
			'refunds',
		);
        ...
        add_action('woocommerce_api_wc_gateway_eu', [$this, 'pay']);
	...
	public static function log( $message, $level = 'info' ) {
		if ( self::$log_enabled ) {
			...
			self::$log->log( $level, $message, array( 'source' => 'expressunion' ) );
		}
	}
    ...
    function pay( $order_id ) {
        ...
        switch ( $status ) {
			case 'OK' :
				...
				update_post_meta( $order->get_id(), '_expressunion_status', $status );
	...
	public function get_icon() {
		...
		foreach ( $icon as $i ) {
			$icon_html .= '<img width="100px" src="' . esc_attr( $i ) . '" alt="' . esc_attr__( 'Afrikpay acceptance mark', 'woocommerce' ) . '" />';
		}
	...
	public function is_valid_for_use() {
		return in_array( get_woocommerce_currency(), apply_filters( 'woocommerce_expressunion_supported_currencies', array( 'XAF','USD','EUR' ) ) );
	}
	...
	public function init_form_fields() {
		$this->form_fields = include( 'settings-expressunion.php' );
	}
	...
	public function process_payment( $order_id ) {
		include_once( 'class-wc-gateway-expressunion-request.php' );

		$order          = wc_get_order( $order_id );
		$momo_request = new WC_Gateway_EU_Request( $this );
	...
	public function capture_payment( $order_id ) {
		$order = wc_get_order( $order_id );
		if ( 'expressunion' === $order->get_payment_method() && 'pending' === get_post_meta( $order->get_id(), '_expressunion_status', true ) && $order->get_transaction_id() ) {
