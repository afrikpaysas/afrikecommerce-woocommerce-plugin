 ...
 */
define( 'WC_PAYPAL_PAYMENTS_VERSION', '1.0' );
define( 'WC_PAYPAL_PAYMENTS_MIN_PHP_VER', '5.4.0' );
define( 'WC_PAYPAL_PAYMENTS_MIN_WC_VER', '3.0.0' );
define( 'WC_PAYPAL_PAYMENTS_MAIN_FILE', __FILE__ );
define( 'WC_PAYPAL_PAYMENTS_PLUGIN_PATH', untrailingslashit( plugin_dir_path( __FILE__ ) ) );

if ( ! class_exists( 'WC_PAYPAL' ) ) {
	...
	class WC_PAYPAL {
		...
		public function order_management_check() {
			if ( ! class_exists( 'WC_PAYPAL_Order_Management' ) ) {
				$current_screen = get_current_screen();
				if ( 'shop_order' === $current_screen->id || 'plugins' === $current_screen->id || 'woocommerce_page_wc-settings' === $current_screen->id ) {
					?>
					<div class="notice notice-warning">
						<p><?php _e( 'Afrikpay Order Management is not active. Please activate it so you can capture, cancel, update and refund Afrikpay orders.', 'woocommerce-paypal-payments' ); ?></p>
					</div>
					<?php
				}
			}
		...
		public function plugin_action_links( $links ) {
			...
			$plugin_links = array(
				'<a href="' . $setting_link . '">' . __( 'Settings', 'paypal-payments-for-woocommerce' ) . '</a>',
				'<a href="http://afrikpay.cederconsulting.com/">' . __( 'Support', 'paypal-payments-for-woocommerce' ) . '</a>',
			);
		...
		public function get_setting_link() {
			...
			$section_slug = $use_id_as_section ? 'paypal' : strtolower( 'WC_Gateway_PAYPAL' );
		...
		public function init_gateways() {
			...
			include_once( WC_PAYPAL_PAYMENTS_PLUGIN_PATH . '/includes/class-wc-gateway-paypal-payments.php' );
			//include_once( WC_PAYPAL_PAYMENTS_PLUGIN_PATH . '/includes/class-wc-afrikpay-payments-order-lines.php' );

			load_plugin_textdomain( 'paypal-payments-for-woocommerce', false, plugin_basename( dirname( __FILE__ ) ) . '/languages' );
		...
		public static function log( $message ) {
			...
			self::$log->add( 'paypal-payments-for-woocommerce', $message );
		}
		...
		public function maybe_filter_billing_phone( $phone_value ) {
			...
			if ( 'paypal_payments' === $_POST['payment_method'] ) { // Input var okay.
